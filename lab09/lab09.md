...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   ...
   Адрес моего хоста 192.168.0.110, адрес хоста назначения 140.82.121.4.

2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?

   В ICMP-пакетах нет номеров портов, номера портов указываются в протоколах транспортного уровня (TCP, UDP), ICMP это протокол сетевого уровня.

3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   
   Тип 8, Echo (ping) request, кодовый номер 0. Другие поля: checksum (контрольная сумма), identifier (идентификатор), sequence number (порядковый номер), timestamp (время), data (данные). Контрольная сумма, порядковый номер и идетификатор занимают по 2 байта каждый.
...
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   Тип 0, Echo (ping) reply, кодовый номер 0. Другие поля  такие же, как в echo request пакете: checksum (контрольная сумма), identifier (идентификатор), sequence number (порядковый номер), timestamp (время), data (данные). Контрольная сумма, порядковый номер и идетификатор занимают по 2 байта каждый.
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   Отличается тем что у него Time To Live выставлено в 1 (у пакетов, посылаемых утилитой ping, оно было 64), и тем что порядковый номер равен 0 (для ping был 1). Ещё секция с данными у этих пакетов короче -- для traceroute она занимает 24 байта, для ping занимала 40 байт.
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   В ICMP-пакете с сообщением об ошибке кроме тех полей, которые были и в пакете с эхо-запросом, содержатся все данные пакета с эхо-запросом (его IP header, ICMP header, сами данные).
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   Три последних пакета -- ответы на успешные запросы, а не сообщения об ошибке. Программа traceroute посылала эхо-запросы, постепенно увеличивая TTL, и после того как достигла такого значения TTL, которого хватает чтобы пакеты дошли до точки назначения, завершает работу -- и она определяет что такой момент наступил когда получает 3 успешных ответа на запросы.

   <img src="images/Screenshot-16.png" width=1000 />   
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   Самая большая задержка была между 6 и 7 маршрутизаторами, 139.45.238.84 (в Москве) и 87.245.233.233 (в Нидерландах) (разница примерно в 37 миллисекунд). Что немного странно, потому что разница в задержке между двумя последними маршрутизаторами (10 и 13), 87.119.94.70 (в Германии) и 140.82.121.4 (в Америке) была в среднем примерно 5 миллисекунд. То есть бОльшая часть задержки обусловлена НЕ тем фактом, что хост на другом континете.
