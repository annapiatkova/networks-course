...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   В IP-заголовке 20 байт, в UDP-заголовке 8 байт, полезная нагрузка UDP 28 байт. С точки зрения IP-протокола UDP-заголовок тоже входит в полезную нагрузку, получаем 8 + 28 = 36 байт (полезная нагрузка IP-дейтаграммы).
...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      Меняются поля identification, time to live (оно не каждый раз изменяется, а инкрементируется каждые 3 пакета), и header checksum.
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      В рамках одного запуска утилиты не должны меняться поля version, header length, differentiated services field, protocol, source address, и destination address (а вот total length, flags, и fragment offset могут меняться если увеличить размер UDP-пакета так чтобы они фрагментировались, это то что происходит в 8 пункте задания)

      Должны изменяться поля identification (меняется между UDP-пакетами), TTL (меняется каждые 3 UDP пакета (по умолчанию, вообще это параметр утилиты traceroute, можно отправлять и не по 3)), checksum (меняется между IP-пакетами).
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      Нет закономерности (значения 0x315a, 0x7492, 0x1870, 0x4918, 0x1cd4, 0xcffc, 0x3e6e, 0xe74c etc)

5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?

   Identification = 0x315a (12634), TTL = 1.
...
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   В ICMP-отклике содержатся identification и TTL пакета, на который отправлен этот отклик (identification = 0x315a, TTL = 1). Кроме того, ICMP-отклик тоже отправляется по протоколу IP и соответственно у него есть свои identification и TTL (identification = 0x149d, TTL = 64). 
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      Все отправляемые нами UDP-пакеты фрагментируются между 3 IP-дейтаграммами каждый.

   2. Какие поля IP-заголовка изменяются в разных фрагментах?

      Total length отличается в последнем из 3 фрагментов, он меньше чем предыдущие 2 (была 1500, стала 540), также в последнем фрагменте отличаются flags -- в предыдущих двух был выставлен флаг что дальше будут ещё фрагменты (flags: 0x1, more fragments), в последнем фрагменте соответственно этот флаг не выставлен (flags: 0x0), меняется fragment offset (увеличивается в каждом следующем пакете на размер предыдущего), и меняется header checksum (поскольку меняются другие поля, то и контрольная сумма получается каждый раз разная).
