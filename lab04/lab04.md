...
#### Демонстрация работы
   (Только первая часть, GET-запросы пишутся в логфайл)
...
1. Найдите DNS-запрос и ответ на него. С использованием какого транспортного протокола
   ...
   DNS-запрос и ответ на него отправлены с использованием протокола UDP.
...
3. На какой IP-адрес отправлен DNS-запрос? Используйте ipconfig для определения IP-адреса
   ...
   DNS-запрос отправлен на адрес 192.168.0.1, адрес локального DNS-сервера 127.0.0.53.

4. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?

   Запрашивает запись типа A (1) (Host Address). "Ответов" в запросе не содержится.
...
5. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   ...
   2 ответа, в них содержатся IP-адреса 104.16.45.99 и 104.16.44.99.
...
6. Посмотрите на последующий TCP-пакет с флагом SYN, отправленный вашим компьютером.
   ...
   Да, он отправлен на один из адресов из ответного сообщения DNS, 104.16.44.99.

7. Веб-страница содержит изображения. Выполняет ли хост новые запросы DNS перед
   загрузкой этих изображений?

   DNS-запросов несколько, после DNS-запроса на www.ietf.org потом отправляется ещё DNS-запрос на static.ietf.org, но отдельных запросов для изображений нет
...
1. Каков порт назначения в запросе DNS? Какой порт источника в DNS-ответе?
   ...
   Порт назначения в запросе и порт источника в ответе 53.

2. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию?

   DNS-запрос отправлен на адрес 127.0.0.53, это адрес локального DNS-сервера.

3. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?

   Запрашивается запись типа AAA (28) (IP6 Address), "ответов" в запросе не содержится.
...
4. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в каждом?
   ...
   В ответном сообщениив в последней паре DNS-запрос, ответ на него тоже нет "ответов".

   Есть "ответ" в одной из предыдущих пар, там в "ответе" содержится адрес 195.70.219.100, который и вывела команда nslookup:
