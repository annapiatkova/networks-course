...
3. Значение в поле Length (Длина) – это длина чего?
   ...
   Длина всей UDP-дейтаграммы (header + payload) в байтах. Header занимает 8 байт, payload занимает 41 байт.

4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   
   Поскольку длина в байтах должна поместиться в поле длины 16 бит в UDP-заголовке, она не может превышать $2^{16} - 1 = 65535$ байт. При этом 8 байт будет занимать сам заголовок, поэтому на payload останется не больше $65535 - 8 = 65527$ байт.
...
5. Чему равно максимально возможное значение номера порта отправителя?
   ...
   Порт отправителя записывается в поле длины 16 бит в UDP-заголовке, поэтому его максимальное возможное значение $2^{16} - 1 = 65535$.
